...
const Card: React.FC<CardProps> = ({
  ...
  const cardClasses = `bg-dark-800 rounded-lg shadow-lg ${bordered ? 'border border-dark-700' : ''} ${className}`;
  const headerClasses = 'flex items-center justify-between mb-4';
  const titleClasses = `text-lg font-semibold text-white ${titleClassName}`;
  const contentClasses = `${noPadding ? '' : 'px-5 py-4'} ${contentClassName}`;
  const footerClasses = 'mt-4 pt-4 border-t border-dark-700';
